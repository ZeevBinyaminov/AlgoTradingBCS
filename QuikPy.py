...
class QuikPy(metaclass=Singleton):  # Singleton класс
    ...
    def __init__(self, host='127.0.0.1', requests_port=34130, callbacks_port=34131):
        ...
        self.OnFirm = self.default_handler
  # 1. Новая фирма
        self.OnAllTrade = self.default_handler
  # 2. Получение обезличенной сделки
        self.OnTrade = self.default_handler
  # 3. Получение новой / изменение существующей сделки
        self.OnOrder = self.default_handler
  # 4. Получение новой / изменение существующей заявки
        self.OnAccountBalance = self.default_handler
  # 5. Изменение позиций
        self.OnFuturesLimitChange = self.default_handler
  # 6. Изменение ограничений по срочному рынку
        self.OnFuturesLimitDelete = self.default_handler
  # 7. Удаление ограничений по срочному рынку
        self.OnFuturesClientHolding = self.default_handler
  # 8. Изменение позиции по срочному рынку
        self.OnMoneyLimit = self.default_handler
  # 9. Изменение денежной позиции
        self.OnMoneyLimitDelete = self.default_handler
  # 10. Удаление денежной позиции
        self.OnDepoLimit = self.default_handler
  # 11. Изменение позиций по инструментам
        self.OnDepoLimitDelete = self.default_handler
  # 12. Удаление позиции по инструментам
        self.OnAccountPosition = self.default_handler
  # 13. Изменение денежных средств
        # OnNegDeal - 14. Получение новой / изменение существующей внебиржевой заявки
        # OnNegTrade - 15. Получение новой / изменение существующей сделки для исполнения
        self.OnStopOrder = self.default_handler
  # 16. Получение новой / изменение существующей стоп-заявки
        self.OnTransReply = self.default_handler
  # 17. Ответ на транзакцию пользователя
        self.OnParam = self.default_handler
  # 18. Изменение текущих параметров
        self.OnQuote = self.default_handler
  # 19. Изменение стакана котировок
        self.OnDisconnected = self.default_handler
  # 20. Отключение терминала от сервера QUIK
        self.OnConnected = self.default_handler
  # 21. Соединение терминала с сервером QUIK
        # OnCleanUp - 22. Смена сервера QUIK / Пользователя / Сессии
        self.OnClose = self.default_handler
  # 23. Закрытие терминала QUIK
        self.OnStop = self.default_handler
  # 24. Остановка LUA скрипта в терминале QUIK / закрытие терминала QUIK
        self.OnInit = self.default_handler
  # 25. Запуск LUA скрипта в терминале QUIK

        # Функции обратного вызова QuikSharp
        self.OnNewCandle = self.default_handler
  # Получение новой свечки
        self.OnError = self.default_handler
